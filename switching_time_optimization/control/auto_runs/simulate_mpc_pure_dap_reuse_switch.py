...
logfile = open("../results/current_run_og_"+str(model_sys)+".txt","w")
...
# Load history
filename = '../results/sim_history/og_history_(2018-01-01 12:00:00)_(100_days)' + '_(price_slope_' + str(price_slope) +  ')_(regime_slope_' + str(regime_slope) +  ')_(seed_1235)_(n_s_' + str(n_s) + ')_(sys_model_' + sys_mod + ')_(sys_true_m3).npy'
histories = np.load(filename,allow_pickle=True).item()


for day in range(n_days):
    print('  ----- Simulating day ' + str(day) + ' ----  ')
    logfile = open("../results/current_run_og_"+str(model_sys)+".txt","a")
    ...
    # Load optimal switches
    opt_switch = histories['SWITCHES_dap'][day]

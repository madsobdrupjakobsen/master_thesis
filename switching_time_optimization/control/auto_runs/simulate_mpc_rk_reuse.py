...
for day in range(n_days):
    ...
    # Load optimal switche
    #opt_switch = tank.get_p_optimize_ipopt()
    switch_promised =  histories['SWITCHES_promised'][day] #opt_switch[:2*n_s]
    switch_used =  histories['SWITCHES_rk'][day] #opt_switch[2*n_s:]
    ...
    filenname = '../results/sim_history/rk_history_reuse(' + start_date + ')_(' + str(n_days) + '_days)' + '_(price_slope_' + str(price_slope) +  ')_(regime_slope_' + str(regime_slope) +  ')_(seed_' + str(seed) +  ')_(n_s_' + str(n_s) + ')_(sys_model_' +  sys_mod + ')_(sys_true_' + sys_true + ')' +  '.npy'

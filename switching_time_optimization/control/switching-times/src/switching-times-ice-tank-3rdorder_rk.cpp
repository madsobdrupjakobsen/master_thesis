...
namespace SwitchingTimes {
    ...
    void Plant::model(const vector<scalar> &x, vector<scalar> &dxdt,
        ...
        for(int k = 0; k < n_opt; ++k) {
            model_regime += 1. / ((1. + cexp(-p_const(15) * (t - on(k)), 15.)) *
                                  (1. + cexp( p_const(15) * (t - off(k)), 15.)));
        };
        /*
        ...
        scalar _price = 0.;
        for(int k = 0; k < 48; ++k) {
            _price += price(k) / ((1. + cexp(-p_const(14) * (t - dat(k)), 15.)) *
                                         (1. + cexp( p_const(14) * (t - dat(k + 1)), 15.)));
        };
        /*
         * Compute dynamics
         * 
         */
        dxdt(0) =  x(1) / p_const(8);
        dxdt(1) =  x(2) / p_const(8);
        dxdt(2) =  1    / p_const(8) * ( 
            model_regime * (-p_const(7) * x(2) - p_const(5) * x(1) + p_const(3) * (p_const(1) - x(0)) ) + 
         (1-model_regime) * (-p_const(6)   * x(2) - p_const(4) * x(1) + p_const(2) * (p_const(0) - x(0)) )
        );
        
        dxdt(3) =   (p_const(11) * 1./(1. + cexp(-(p_const(8) * (x(0) - p_const(9))),15)) +
                                    model_regime * p_const(12) + 
                                    (1-model_regime) * p_const(13)) * 1./60.;                         // Cost

        dxdt(4) =  _price * (p_const(11) * 1./(1. + cexp(-(p_const(8) * (x(0) - p_const(9))),15)) +
                                    model_regime * p_const(12) + 
                                    (1-model_regime) * p_const(13)) * 1./60.;                         // Cost

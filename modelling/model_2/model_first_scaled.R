model <- function(){
  ...
  model$addSystem(dX0 ~ (X1) * dt + exp(p11)*dw1)
  model$addSystem(dX1 ~ (status * (-2 * xi1 * omega1 * (X1) - omega1**2 * (X0-698.861205845*mu1) ) + 
                           (1-status)*(-2 * xi0 * omega0 * (X1) - omega0**2 * (X0-961.996347735*mu0) ))  * dt + 0*dw2)
  ...
  model$addObs(CompCap ~ 1./(1. + exp(-(slope * (X0 - 592.010123492*offset)))) )  #(1/(1+exp((setPoint-T2)*sigmoidSlope)))) 

model <- function(){
  ...
  model$addSystem(dX1 ~ (status * (-2 * xi1 * omega1 * (X1) - omega1**2 * (X0-500*mu1) ) + 
                           (1-status)*(-2 * xi0 * omega0 * (X1) - omega0**2 * (X0-700*mu0) ))  * dt + exp(p22)*dw2)
  ...
  model$addObs(CompCap ~ 1./(1. + exp(-(slope * (X0 - 500*offset)))) )  #(1/(1+exp((setPoint-T2)*sigmoidSlope)))) 

...
namespace SwitchingTimes {
    ...
    public:
        ...
        scalar objective_wrapper(const vector<scalar> &p_dynamic_x0, const vector<scalar> &p_opt) {
            ...
            vector<scalar> dap = vector<scalar>::Zero(48);
            for(int k = 0; k < 48; ++k) { dap(k) = _p_dynamic(k); };

            vector<scalar> rk = vector<scalar>::Zero(48);
            for(int k = 0; k < 48; ++k) { rk(k) = _p_dynamic(48 + k); };

            vector<scalar> dat = vector<scalar>::Zero(48);
            for(int k = 0; k < 48; ++k) { dat(k) = _p_dynamic(48 + 48 + k); };

            vector<scalar> prob_rk = vector<scalar>::Zero(1);
            prob_rk(0) = _p_dynamic(_p_dynamic.size());

             // Derive the two switching time paths
            size_t n_s = p_opt.size() / 2;
            ...
            for(int k = 0; k < 48; ++k) {
                ...
                integrate_const(rk5_stepper,
                                           [&] (const vector<scalar> &x , vector<scalar> &dxdt , const double t) {
                                               model(x, dxdt, t, p_dynamic_x0, p_opt_dap, _p_const);
                                           }, x_dap, _t0_sub, _tf_sub, _dt);
                ...
                integrate_const(rk5_stepper,
                                           ...
                                           }, x_rk, _t0_sub, _tf_sub, _dt);
                                           

                _t0_sub += total_int_time/48.;
                _tf_sub += total_int_time/48.;

                

                total_price(0) +=  prob_rk(0) * (x_dap(1) * dap(k)) + ((x_rk(1) - x_dap(1)) * rk(k)) + 
                                   (1-prob_rk(0)) * (x_dap(1) * dap(k));
        ...
        double objective_wrapper(const vector<double> &p_opt) {
            ...
            vector<double> dap = vector<double>::Zero(48);
            for(int k = 0; k < 48; ++k) { dap(k) = _p_dynamic(k); };

            vector<double> rk = vector<double>::Zero(48);
            for(int k = 0; k < 48; ++k) { rk(k) = _p_dynamic(48 + k); };

            vector<double> dat = vector<double>::Zero(48);
            for(int k = 0; k < 48; ++k) { dat(k) = _p_dynamic(48 + 48 + k); };
            ...
            for(int k = 0; k < 48; ++k) {
                ...
                integrate_const(rk5_stepper,
                                            [&] (const vector<double> &x , vector<double> &dxdt , const double t) {
                                                model(x, dxdt, t, _p_dynamic, p_opt_dap, _p_const);
                                            }, x_dap, _t0_sub, _tf_sub, _dt);
